    ...
    public partial class MainForm : Form
    {
        ...
        private RenamerOptions GetRenamerOptions()
        {
            return new RenamerOptions()
            {
                ...
                SourceSolutionName = fromText.Text,
                DestSolutionName = toText.Text,
                RenameDirectories = renameDirectories.Checked,
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    if (String.IsNullOrEmpty(renamerOptions.DestSolutionName))
                    {
                    ...
                    var webConfigOptions = new WebConfigOptions()
                    {
                        WebConfigPath = Path.Combine(renamerOptions.SourceDirectory, $"{renamerOptions.DestSolutionName}.Web", "Web.config"),
                        SolutionName = renamerOptions.DestSolutionName
                    };

                    var webconfig = new WebConfigSetup(webConfigOptions, logger);
                    webconfig.Run();

                    var appSettingsOptions = new AppSettingsOptions()
                    {
                        WebProjectFolder = Path.Combine(renamerOptions.SourceDirectory, $"{renamerOptions.DestSolutionName}.Web"),
                        SolutionName = renamerOptions.DestSolutionName
                    };

                    var appSettingsConfig = new AppSettingsSetup(appSettingsOptions, logger);
                    appSettingsConfig.Run();
