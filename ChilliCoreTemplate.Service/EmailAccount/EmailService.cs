...
public class EmailService(IPrincipal user, DataContext context, ProjectSettings config, IFileStorage fileStorage, IWebHostEnvironment environment, IMapper mapper, ITemplateViewRenderer templateViewRenderer, FileStoragePath fileStoragePath)
    : BaseService(user, context, config, fileStorage, environment, mapper)
{
    private readonly ITemplateViewRenderer _templateViewRenderer = templateViewRenderer;
    private readonly FileStoragePath _fileStoragePath = fileStoragePath;

    internal static Guid FakeTrackingId => new("D547C67A-EB4D-4EA9-AF51-077E1389FECB");

    public ServiceResult<EmailListModel> List(int? userId)
    {
    ...
    public PagedList<EmailSummaryModel> Search(IDataTablesRequest model, DateTime dateFrom, DateTime dateTo, int? userId)
    {
    ...
    internal ServiceResult<EmailViewModel> Email_Preview<T>(EmailPreviewItemModel email) where T : class
    {
        ...
        html = MjmlToHtmlHelper.Render(html);
...
public partial class AccountService
{
    ...
    {
        if (id.Guid == EmailService.FakeTrackingId) return ServiceResult.AsSuccess();
