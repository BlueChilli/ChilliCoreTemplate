...
{
    private static StripeService GetStripeService(IServiceScope scope)
    {
        return scope.ServiceProvider.GetRequiredService<StripeService>();
    }
    private static DataContext GetDataContext(IServiceScope scope)
    {
        return scope.ServiceProvider.GetRequiredService<DataContext>();
    }
    private static EmailAccount.EmailQueueService GetEmailService(IServiceScope scope)
    {
        return scope.ServiceProvider.GetRequiredService<EmailAccount.EmailQueueService>();
    }
    private static ProjectSettings GetProjectSettings(IServiceScope scope)
    {
        return scope.ServiceProvider.GetRequiredService<ProjectSettings>();
    }

    private static bool CheckSystemMetadata(Dictionary<string, string> metadata, ProjectSettings config)
    {
        if (metadata.TryGetValue(StripeService.SYSTEM, out var baseUrl))
        {
    ...
    private static async Task<ServiceResult> ProcessChargeRefunded(Stripe.Event stripeEvent, IServiceScope scope)
    {
        var context = GetDataContext(scope);
    ...
    private static async Task<ServiceResult> ProcessChargeSucceeded(Stripe.Event stripeEvent, IServiceScope scope)
    {
        string jsonData = JsonConvert.SerializeObject(stripeEvent.Data.Object);
    ...
    private static async Task<ServiceResult> ProcessInvoicePaid(Stripe.Event stripeEvent, IServiceScope scope)
    {
    ...
    private static async Task<ServiceResult> ProcessInvoicePaymentFailed(Stripe.Event stripeEvent, IServiceScope scope)
    {
        var model = stripeEvent.Data.Object as Invoice;
